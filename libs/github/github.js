  ...
  Github = window.Github = function(options) {
    ...
    function _request(method, path, data, cb, raw) {
      function getURL() {
        ...
        return url + ((/\?/).test(url) ? "&" : "?callback=null&") + (new Date()).getTime();
      }
    ...
    Github.Repository = function(options) {
      ...
      this.milestones = function(cb)
      {
          var state = "open";
          var url = "/repos/"+user+"/"+repo+"/milestones?state="+state;
          _request("GET", url, null, function(err, res) {
            cb(err, res);
          });
      }
      
      this.issues = function(milestone, cb)
      {
          var state = "open";
          var assignee = "*";
          var labels = "HubTime"; 
          
          var url = "/repos/"+user+"/"+repo+"/issues?state="+state+"&labels="+labels+"&milestone="+milestone;  
      ...
      this.issueComments = function(issue, cb)
      {
      ...
      this.addIssueComment = function(issue, comment, cb)
      {
          var url = "/repos/"+user+"/"+repo+"/issues/"+issue+"/comments";
          _request("POST", url, {"body": comment}, function(err, res) {
              cb(err, res);
          });
      }
      
      this.removeIssueComment = function(commentID, cb)
      {
          var url = "/repos/"+user+"/"+repo+"/issues/comments/"+commentID;
          _request("DELETE", url, null, function(err, res) {
              cb(err, res);
          });
      }
